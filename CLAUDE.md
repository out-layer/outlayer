...
- **Auto-opening a wallet**: never call `signAndSendTransaction` — or anything that opens a wallet — from a promise chain, an effect, a redirect callback or a timer. Only a real user click counts as a gesture; anything else fails inside the wallet with an error nobody can act on (Meteor answers `reading 'M_ID'`). Split the flow AT the signature: do the unsigned work, then stop and show what happened and what the transaction will do — what is stored, under whose account, who can read it, how to undo it — and put the action in the button's label. A refused wallet returns to that screen, not to the start
